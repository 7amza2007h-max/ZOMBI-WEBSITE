...
async function guildPage(req){
  ...
  const directorTemplateOptions=directorTemplates.filter(x=>x.enabled!==false).map(x=>`<option value="${esc(x.id)}">${esc((x.emoji||'🌆')+' '+(x.name||x.id))}</option>`).join('');
  ...
  <form class="panel" method="post" action="/dashboard/${guild.id}/settings"><input type="hidden" name="_csrf" value="${token}">
    ...
    <h3>🌆 ZOMBI City Director</h3><div class="form-grid"><label><input type="checkbox" name="cityDirectorEnabled" ${cfg.cityDirector?.enabled?'checked':''}> تشغيل النظام</label><label><input type="checkbox" name="cityDirectorAutoEnabled" ${cfg.cityDirector?.autoEnabled?'checked':''}> أحداث تلقائية</label><label>بين كل حدث وحدث (دقيقة)<input type="number" name="cityDirectorIntervalMinutes" value="${Number(cfg.cityDirector?.intervalMinutes||120)}" min="5" max="10080"></label><label>مدة الحدث (دقيقة)<input type="number" name="cityDirectorDurationMinutes" value="${Number(cfg.cityDirector?.durationMinutes||10)}" min="1" max="180"></label><label>الهدف الأساسي<input type="number" name="cityDirectorGoal" value="${Number(cfg.cityDirector?.goal||20)}" min="3" max="10000"></label><label>أقصى مشاركين<input type="number" name="cityDirectorMaxParticipants" value="${Number(cfg.cityDirector?.maxParticipants||30)}" min="1" max="500"></label><label>أقل جائزة<input type="number" name="cityDirectorRewardMin" value="${Number(cfg.cityDirector?.rewardMin||500)}" min="0"></label><label>أعلى جائزة<input type="number" name="cityDirectorRewardMax" value="${Number(cfg.cityDirector?.rewardMax||1500)}" min="0"></label><label><input type="checkbox" name="cityDirectorMentionEveryone" ${cfg.cityDirector?.mentionEveryone?'checked':''}> منشن @everyone عند بداية الحدث</label></div>
...
async function start(){
  ...
  app.post('/dashboard/:guildId/settings',requireLogin,requireGuildAccess,checkCsrf,async(req,res,next)=>{try{
    ...
    if(saves('director')){
      ...
      cfg.cityDirector={...cfg.cityDirector,enabled:Boolean(req.body.cityDirectorEnabled),autoEnabled:Boolean(req.body.cityDirectorAutoEnabled),intervalMinutes:int(req.body.cityDirectorIntervalMinutes,cfg.cityDirector?.intervalMinutes||120,5,10080),durationMinutes:int(req.body.cityDirectorDurationMinutes,cfg.cityDirector?.durationMinutes||10,1,180),goal:int(req.body.cityDirectorGoal,cfg.cityDirector?.goal||20,3,10000),maxParticipants:int(req.body.cityDirectorMaxParticipants,cfg.cityDirector?.maxParticipants||30,1,500),rewardMin:int(req.body.cityDirectorRewardMin,cfg.cityDirector?.rewardMin||500,0,1000000000),rewardMax:int(req.body.cityDirectorRewardMax,cfg.cityDirector?.rewardMax||1500,0,1000000000),mentionEveryone:Boolean(req.body.cityDirectorMentionEveryone)};
  ...
  app.post('/dashboard/:guildId/city-director/start',requireLogin,requireGuildAccess,checkCsrf,async(req,res,next)=>{try{
    ...
    const wanted=String(req.body.templateId||'');if(wanted&&!(cfg.cityDirector.templates||[]).some(x=>x.id===wanted&&x.enabled!==false))return res.status(400).send('قالب الحدث غير صالح أو غير مفعّل.');
