...
async function guildPage(req){
  ...
  <form class="panel" method="post" action="/dashboard/${guild.id}/settings"><input type="hidden" name="_csrf" value="${token}">
    ...
    <h3>🎵 نظام الموسيقى في الفويس</h3>${lockedNote(canFeature('music'),'نظام الموسيقى غير متاح في خطة هذا السيرفر حسب إعدادات Owner.')}<div class="z-game-command-guide"><b>🎧 ZOMBI MUSIC CENTER</b><p>اكتب <code>فويس</code> داخل <b>شات نفس الروم الصوتي</b> لتظهر لوحة الموسيقى. يعمل مع أي Voice عادي أو مؤقت، ولا يعمل داخل الشاتات النصية العادية.</p><small>الأزرار مرتبطة بنفس الفويس: تشغيل YouTube، إيقاف/استكمال، تخطي، تكرار، Queue، تنظيف الطابور، رفع/خفض الصوت وخروج البوت.</small></div><div class="form-grid"><label><input type="checkbox" name="musicEnabled" ${cfg.music?.enabled!==false?'checked':''} ${disabled(canFeature('music'))}> تشغيل نظام الموسيقى في هذا السيرفر</label><label><input type="checkbox" name="musicAllowEveryone" ${cfg.music?.allowEveryone!==false?'checked':''} ${disabled(canFeature('music'))}> السماح لكل أعضاء نفس الفويس بالتحكم</label><label><input type="checkbox" name="musicAnnounceNowPlaying" ${cfg.music?.announceNowPlaying!==false?'checked':''} ${disabled(canFeature('music'))}> إرسال إشعار "الآن يعمل" داخل شات الفويس</label><label>الصوت الافتراضي<input type="number" name="musicDefaultVolume" value="${Math.min(Number(cfg.music?.defaultVolume||60),maxFor(req,cfg,site,'musicMaxVolume'))}" min="1" max="${maxFor(req,cfg,site,'musicMaxVolume')}" data-plan-max="${maxFor(req,cfg,site,'musicMaxVolume')}" data-limit-label="مستوى صوت الموسيقى" ${disabled(canFeature('music'))}></label><label>الخروج التلقائي بعد التوقف — ثانية<input type="number" name="musicAutoLeaveSeconds" value="${Number(cfg.music?.autoLeaveSeconds||180)}" min="30" max="3600" ${disabled(canFeature('music'))}></label><label class="wide">رتب DJ المسموح لها بالتحكم<select multiple name="musicControllerRoleIds" ${disabled(canFeature('music'))}>${roleOptions(roles,guild.id,cfg.music?.controllerRoleIds)}</select><small class="hint">إذا ألغيت السماح للجميع، يستطيع الأدمن وManage Server دائمًا التحكم، بالإضافة إلى رتب DJ التي تختارها هنا.</small></label></div><div class="checks"><label class="${canFeature('musicQueue')?'':'locked'}">📜 Queue: ${canFeature('musicQueue')?'✅ متاح':'🔒 غير متاح في الخطة'}</label><label class="${canFeature('musicLoop')?'':'locked'}">🔁 Loop: ${canFeature('musicLoop')?'✅ متاح':'🔒 غير متاح في الخطة'}</label><label class="${canFeature('musicSearch')?'':'locked'}">🔎 البحث باسم الأغنية: ${canFeature('musicSearch')?'✅ متاح':'🔒 رابط YouTube فقط'}</label></div><div class="warn small">حدود الخطة الحالية: Queue حتى <b>${maxFor(req,cfg,site,'musicQueueSize')}</b> أغنية • الصوت حتى <b>${maxFor(req,cfg,site,'musicMaxVolume')}%</b> • مدة الأغنية حتى <b>${maxFor(req,cfg,site,'musicMaxTrackMinutes')}</b> دقيقة. الـOwner يغيّر هذه الحدود لكل Free / Premium / Premium+.</div>
...
async function start(){
  ...
  app.post('/dashboard/:guildId/settings',requireLogin,requireGuildAccess,checkCsrf,async(req,res,next)=>{try{
    ...
    const knownSections=new Set(['all','warnings','logs','overview','economy','members','store','games','city','heist','gangs','robbery','roles','name','tickets','music','voice']);
    ...
    if(saves('music')){
      if(!featureAllowed(site,cfg,'music'))return sendUpgradeRequired(req,res,site,cfg,'نظام الموسيقى غير متاح في خطة هذا السيرفر حسب إعدادات Owner.');
      const musicRoles=arr(req.body.musicControllerRoleIds).map(x=>String(x).trim()).filter(Boolean);
      if(musicRoles.some(id=>!req.bundle.roles.some(r=>String(r.id)===id&&!r.managed&&String(r.id)!==String(req.params.guildId))))return res.status(400).send('رتبة DJ غير صالحة لهذا السيرفر.');
      cfg.music={
        ...(cfg.music||{}),
        enabled:Boolean(req.body.musicEnabled),
        allowEveryone:Boolean(req.body.musicAllowEveryone),
        announceNowPlaying:Boolean(req.body.musicAnnounceNowPlaying),
        defaultVolume:int(req.body.musicDefaultVolume,cfg.music?.defaultVolume||60,1,maxFor(req,cfg,site,'musicMaxVolume')),
        autoLeaveSeconds:int(req.body.musicAutoLeaveSeconds,cfg.music?.autoLeaveSeconds||180,30,3600),
        controllerRoleIds:musicRoles.slice(0,50)
      };
    }
