...
function makeDashboardPanelV2(payload,centerName='ZOMB • CONTROL CENTER',marker='ZOMB-PANEL-V2'){
  ...
  const parts=[
    {type:12,items:[{media:{url:`${assetBase}/zombi-compact-header.png`},description:'ZOMBI'}]},
    {type:9,components:[{type:10,content:`## ${title}\n-# ✦ ${cleanPanelText(centerName,120)} ✦`}],accessory:{type:11,media:{url:`${assetBase}/zombi-orb.gif`},description:'ZOMBI'}},
    {type:14,divider:true,spacing:1}
  ];
  if(body)parts.push({type:10,content:body});
  const rows=Array.isArray(payload?.components)?payload.components.slice(0,5):[];
  if(rows.length){parts.push({type:14,divider:true,spacing:1});parts.push(...rows);}
...
async function sendPanel(which,guildId,bundle,options={}){const [cfg,site]=await Promise.all([options.config||store.getConfig(guildId),store.getGlobalConfig()]);
  ...
  const sendOrUpdate=async(channelId,messageId,payload)=>{
    ...
    const centers={bank:'ZOMB • BANK CENTER',games:'ZOMB • GAMES CENTER',tickets:'ZOMB • SUPPORT CENTER',store:'ZOMB • STORE CENTER',roles:'ZOMB • ROLE CENTER',guide:'ZOMB • SERVER GUIDE',name:'ZOMB • NAME CENTER'};
    payload=makeDashboardPanelV2(payload,centers[which]||'ZOMB • CONTROL CENTER',`ZOMB-${String(which||'panel').toUpperCase()}-PANEL-V2`);
