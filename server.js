...
const CORE_FEATURES=['economy','bank','games','tickets','store','rolePanel','levels','voiceRewards','voiceRooms','moderation','gangs','gangMissions','bankRobbery'];
...
async function guildPage(req){
  ...
  const canFeature=k=>featureAllowed(site,cfg,k), canGameSettings=canFeature('gameSettings'),canQuestions=canFeature('gameQuestions'),canBrand=canFeature('customBranding'),canCurrency=canFeature('customCurrency'),canBotProfile=(isGuildOwner(req)&&featureAllowed(site,cfg,'customBotProfile')),canEconomyAdmin=canFeature('economyAdmin'),profileLockText='هذه الميزة للمشتركين فقط، ولا يستطيع تعديل هوية البوت إلا مالك السيرفر.';
  const storeLimit=maxFor(req,cfg,site,'storeProducts'),roleLimit=maxFor(req,cfg,site,'selfRoles'),ticketLimit=maxFor(req,cfg,site,'ticketTypes'),questionLimit=maxFor(req,cfg,site,'questionsPerGame'),killerLimit=maxFor(req,cfg,site,'killerCases'),missionLimit=maxFor(req,cfg,site,'gangMissionTemplates');
  ...
  <form class="panel" method="post" action="/dashboard/${guild.id}/settings"><input type="hidden" name="_csrf" value="${token}">
    ...
    <h3>🏆 Levels • Profile + XP الاحترافي</h3><div class="form-grid"><label>Chat XP لكل رسالة<input type="number" name="xpPerMessage" value="${cfg.levels.xpPerMessage}" min="1" max="10000"></label><label>Chat XP Cooldown (ثانية)<input type="number" name="xpCooldownSeconds" value="${cfg.levels.xpCooldownSeconds}" min="5" max="3600"></label><label>Voice XP لكل دقيقة<input type="number" name="voiceXpPerMinute" value="${cfg.levels.voiceXpPerMinute??5}" min="0" max="10000"></label><label>أقل أعضاء حقيقيين في الفويس<input type="number" name="voiceMinMembers" value="${cfg.levels.voiceMinMembers??1}" min="1" max="25"></label><label><input type="checkbox" name="voiceIgnoreDeafened" ${cfg.levels.voiceIgnoreDeafened!==false?'checked':''}> لا تحسب XP للـDeafened</label><label><input type="checkbox" name="voiceIgnoreMuted" ${cfg.levels.voiceIgnoreMuted===true?'checked':''}> لا تحسب XP للـMuted</label><label>Base XP<input type="number" name="baseXp" value="${cfg.levels.baseXp}" min="10"></label><label>Growth لكل Level<input type="number" name="levelGrowth" value="${cfg.levels.growth}" min="0"></label><label><input type="checkbox" name="seasonEnabled" ${cfg.levels.seasonEnabled!==false?'checked':''}> تفعيل موسم XP</label><label>اسم الموسم<input name="seasonName" maxlength="80" value="${esc(cfg.levels.seasonName||'Season 1')}"></label><label>نظام رتب المستويات<select name="roleRewardMode"><option value="cumulative" ${cfg.levels.roleRewardMode!=='highest'?'selected':''}>تراكمية — يحتفظ بكل الرتب</option><option value="highest" ${cfg.levels.roleRewardMode==='highest'?'selected':''}>أعلى رتبة فقط</option></select></label><label class="wide">رتب المستويات — Level | Role ID<textarea name="levelRoleRewardsText" placeholder="5 | 123456789012345678
10 | 234567890123456789">${esc((cfg.levels.roleRewards||[]).map(r=>`${r.level} | ${r.roleId}`).join('\n'))}</textarea><small class="hint">مثال: 5 | ID الرتبة. البوت لازم تكون رتبته أعلى من رتب المكافآت.</small></label><div class="wide warn small">أوامر الأعضاء: <code>#p</code> و <code>#p @member</code> و <code>#top</code>. داخل #top توجد أزرار Daily / Weekly / Monthly / All Time و Total / Chat / Voice.</div><button class="btn" type="submit" name="startNewSeason" value="1" onclick="return confirm('بدء موسم XP جديد؟ سيتم تصفير نقاط الموسم فقط وسيبقى All Time محفوظًا.')">🏁 بدء موسم جديد الآن</button></div>
    ...
    <h3>🚨 سرقة البنك المركزي</h3><div class="form-grid"><label><input type="checkbox" name="robberyEnabled" ${cfg.robbery?.enabled===true?'checked':''}> السرقة متاحة الآن</label><label>عدد المشاركين المطلوب<input type="number" name="robberyMinParticipants" value="${cfg.robbery?.minParticipants||5}" min="2" max="${maxFor(req,cfg,site,'robberyParticipants')}"><small>يمكنك تقليل العدد حتى لاعبين اثنين. زر البداية يعتمد على هذا الرقم مباشرة.</small></label><label>المنشن عند فتح السرقة<select name="robberyMentionMode"><option value="none" ${cfg.robbery?.mentionMode==='none'?'selected':''}>بدون منشن</option><option value="everyone" ${!cfg.robbery?.mentionMode||cfg.robbery?.mentionMode==='everyone'?'selected':''}>@everyone</option><option value="here" ${cfg.robbery?.mentionMode==='here'?'selected':''}>@here</option><option value="role" ${cfg.robbery?.mentionMode==='role'?'selected':''}>منشن رتبة محددة</option></select><small>يتم إرسال المنشن مرة واحدة عند فتح سرقة جديدة.</small></label><label data-robbery-mention-role>رتبة المنشن<select name="robberyMentionRoleId"><option value="">اختر الرتبة</option>${roleOptions(roles,guild.id,[cfg.robbery?.mentionRoleId||''])}</select><small>يستخدم فقط عندما تختار «منشن رتبة محددة».</small></label><label>مدة التجمع دقيقة<input type="number" name="robberyLobbyMinutes" value="${cfg.robbery?.lobbyMinutes||10}" min="2"></label><label>مدة المهمات دقيقة<input type="number" name="robberyMissionMinutes" value="${cfg.robbery?.missionMinutes||25}" min="5"></label><label>المكافأة لخزنة العصابة<input type="number" name="robberyReward" value="${cfg.robbery?.reward||50000}" min="1"></label><label>Cooldown ساعات<input type="number" name="robberyCooldownHours" value="${cfg.robbery?.cooldownHours||12}" min="0"></label><label>قناع<input type="number" name="robberyMask" value="${cfg.robbery?.equipment?.mask||0}" min="0"></label><label>جهاز اختراق<input type="number" name="robberyHacking" value="${cfg.robbery?.equipment?.hacking||0}" min="0"></label><label>مثقاب<input type="number" name="robberyDrill" value="${cfg.robbery?.equipment?.drill||0}" min="0"></label><label>جهاز اتصال<input type="number" name="robberyRadio" value="${cfg.robbery?.equipment?.radio||0}" min="0"></label><label>سيارة هروب<input type="number" name="robberyCar" value="${cfg.robbery?.equipment?.car||0}" min="0"></label></div>
    ...
    <h3>📋 سجل السيرفر Log</h3><input type="hidden" name="loggingPresent" value="1"><div class="checks"><label><input type="checkbox" name="modLogActions" ${cfg.moderation?.logActions!==false?'checked':''}> تفعيل سجل السيرفر</label>${Object.entries({audit:'إجراءات الإدارة والرومات والرتب',messages:'حذف وتعديل الرسائل',members:'دخول وخروج وتغييرات الأعضاء',voice:'الفويس والكتم والكاميرا',games:'بدء الألعاب',commands:'استخدام أوامر السلاش',actions:'إجراءات البوت'}).map(([k,label])=>`<label><input type="checkbox" name="log_${k}" ${cfg.logging?.[k]!==false?'checked':''}> ${label}</label>`).join('')}</div><p class="hint">اختر رومًا خاصًا بالإدارة. يحتاج البوت View Audit Log وView Channel وSend Messages وEmbed Links. محتوى الرسائل القديمة قد يكون غير متاح. سجل المشرف منفصل عن الرسالة المحذوفة عندما لا يمكن تأكيد الربط.</p>
    <h3>🛡️ Moderation</h3><div class="checks"><label><input type="checkbox" name="modClearEnabled" ${cfg.moderation?.clearEnabled!==false?'checked':''}> Clear</label><label><input type="checkbox" name="modKickEnabled" ${cfg.moderation?.kickEnabled!==false?'checked':''}> Kick</label><label><input type="checkbox" name="modBanEnabled" ${cfg.moderation?.banEnabled!==false?'checked':''}> Ban</label><label><input type="checkbox" name="modLockEnabled" ${cfg.moderation?.lockEnabled!==false?'checked':''}> Lock</label></div><h3>⚠️ التحذيرات</h3><p class="hint">تحذيرات Premium+ داخل الروم المحدد فقط: <code>/warn</code>، <code>/warnings</code>، <code>/unwarn</code> أو <code>تحذير @العضو السبب</code>. أضف مستويات التحذير واكتب ID رتبة كل مستوى لتطبيقها تلقائيًا.</p><div class="form-grid"><label>روم التحذيرات — Premium+<select name="warningChannelId" ${disabled(planNameForConfig(cfg)==='premium_plus')}>${textChannels(channels,cfg.warnings?.channelId)}</select><small>كل أوامر التحذيرات تعمل في هذا الروم فقط. Premium+ حصريًا.</small></label><label class="wide">مستويات التحذيرات — ID الرتبة لكل مستوى</label><div class="wide" id="warning-levels">${(Array.isArray(cfg.warnings?.roleIds)?cfg.warnings.roleIds:[cfg.warnings?.role1Id||'',cfg.warnings?.role2Id||'',cfg.warnings?.role3Id||'']).map((id,n)=>`<div class="form-grid warning-level-row"><label>التحذير ${n+1} — ID الرتبة<input name="warningRoleIds" value="${esc(id||'')}" inputmode="numeric" pattern="[0-9]{15,25}" placeholder="ID الرتبة" ${disabled(planNameForConfig(cfg)==='premium_plus')}></label><button type="button" class="btn warning-remove-level" ${disabled(planNameForConfig(cfg)==='premium_plus')}>حذف المستوى</button></div>`).join('')}</div><button type="button" class="btn" id="warning-add-level" ${disabled(planNameForConfig(cfg)==='premium_plus')}>＋ إضافة مستوى تحذير</button><p class="hint">الترتيب يحدد التحذير الأول والثاني وما بعدهما. لا يوجد حد ثابت لعدد المستويات. حقل فارغ يعني عدم تعيين رتبة لهذا المستوى؛ بعد آخر مستوى تُستخدم رتبته. لنسخ ID فعّل Developer Mode في ديسكورد ثم اضغط على الرتبة بالزر الأيمن واختر Copy Role ID.</p></div>
  ...
  <section class="panel command-sync"><h2>⚡ Discord Panels & Commands</h2><div class="card-actions"><form method="post" action="/dashboard/${guild.id}/sync-commands"><input type="hidden" name="_csrf" value="${token}"><button class="btn primary">🔄 مزامنة أوامر السيرفر</button></form><form method="post" action="/dashboard/${guild.id}/send/bank"><input type="hidden" name="_csrf" value="${token}"><button class="btn">🏦 لوحة البنك</button></form><form method="post" action="/dashboard/${guild.id}/send/games"><input type="hidden" name="_csrf" value="${token}"><button class="btn">🎮 لوحة الألعاب</button></form><form method="post" action="/dashboard/${guild.id}/send/tickets"><input type="hidden" name="_csrf" value="${token}"><button class="btn">🎫 لوحة التذاكر</button></form><form method="post" action="/dashboard/${guild.id}/send/store"><input type="hidden" name="_csrf" value="${token}"><button class="btn">🛒 لوحة المتجر</button></form><form method="post" action="/dashboard/${guild.id}/send/roles"><input type="hidden" name="_csrf" value="${token}"><button class="btn">🔔 لوحة الرتب</button></form></div></section>
...
async function sendPanel(which,guildId,bundle,options={}){const [cfg,site]=await Promise.all([options.config||store.getConfig(guildId),store.getGlobalConfig()]);
  ...
  const sendOrUpdate=async(channelId,messageId,payload)=>{
    const designCfg=structuredClone(cfg);if(!featureAllowed(site,cfg,'customBotProfile')&&designCfg.panelDesigns?.[which])designCfg.panelDesigns[which].image='';
...
async function start(){
  ...
  app.post('/dashboard/:guildId/settings',requireLogin,requireGuildAccess,checkCsrf,async(req,res,next)=>{try{
    ...
    const knownSections=new Set(['all','warnings','logs','overview','economy','members','store','games','city','heist','gangs','robbery','roles','name','tickets','voice']);
    ...
    if(saves('members')){
      const validRoleIds=new Set((req.bundle.roles||[]).filter(r=>!r.managed&&r.id!==req.params.guildId).map(r=>String(r.id)));
      const roleRewards=String(req.body.levelRoleRewardsText||'').split(/\r?\n/).map(line=>{
        const m=line.match(/^\s*(\d{1,5})\s*[|,;:-]\s*(\d{15,25})\s*$/);
        return m?{level:int(m[1],1,1,10000),roleId:String(m[2])}:null;
      }).filter(x=>x&&validRoleIds.has(x.roleId)).slice(0,50).sort((a,b)=>a.level-b.level);
      cfg.levels={
        ...cfg.levels,
        ...
        voiceXpPerMinute:int(req.body.voiceXpPerMinute,cfg.levels.voiceXpPerMinute??5,0,10000),
        voiceMinMembers:int(req.body.voiceMinMembers,cfg.levels.voiceMinMembers??1,1,25),
        voiceIgnoreMuted:Boolean(req.body.voiceIgnoreMuted),
        voiceIgnoreDeafened:Boolean(req.body.voiceIgnoreDeafened),
        baseXp:int(req.body.baseXp,cfg.levels.baseXp,10,1000000),
        growth:int(req.body.levelGrowth,cfg.levels.growth,0,1000000),
        seasonEnabled:Boolean(req.body.seasonEnabled),
        seasonName:String(req.body.seasonName||cfg.levels.seasonName||'Season 1').slice(0,80),
        seasonId:req.body.startNewSeason?`season-${Date.now()}`:String(cfg.levels.seasonId||'season-1'),
        roleRewardMode:req.body.roleRewardMode==='highest'?'highest':'cumulative',
        roleRewards
      };
      cfg.moderation={
        ...cfg.moderation,
    ...
    if(saves('warnings')){
      if(['warningChannelId','warningRoleIds'].some(has)){
        if(planNameForConfig(cfg)!=='premium_plus')return sendUpgradeRequired(req,res,site,cfg,'نظام التحذيرات يحتاج Premium+ فعّال.');
