...
async function guildPage(req){
  ...
  <form class="panel" method="post" action="/dashboard/${guild.id}/settings"><input type="hidden" name="_csrf" value="${token}">
    ...
    <h3>🛡️ Moderation</h3><div class="checks"><label><input type="checkbox" name="modClearEnabled" ${cfg.moderation?.clearEnabled!==false?'checked':''}> Clear</label><label><input type="checkbox" name="modKickEnabled" ${cfg.moderation?.kickEnabled!==false?'checked':''}> Kick</label><label><input type="checkbox" name="modBanEnabled" ${cfg.moderation?.banEnabled!==false?'checked':''}> Ban</label><label><input type="checkbox" name="modLockEnabled" ${cfg.moderation?.lockEnabled!==false?'checked':''}> Lock</label><label><input type="checkbox" name="modLogActions" ${cfg.moderation?.logActions!==false?'checked':''}> Logs</label></div><div class="form-grid"><label>رتبة التحذير 1<select name="warningRole1Id">${roleOptions(roles,guild.id,[cfg.warnings?.role1Id])}</select></label><label>رتبة التحذير 2<select name="warningRole2Id">${roleOptions(roles,guild.id,[cfg.warnings?.role2Id])}</select></label><label>رتبة التحذير 3<select name="warningRole3Id">${roleOptions(roles,guild.id,[cfg.warnings?.role3Id])}</select></label></div>
