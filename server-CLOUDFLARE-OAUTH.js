...
async function getBotGuilds(){const guilds=await botFetch('/users/@me/guilds?with_counts=true&limit=200');return Array.isArray(guilds)?guilds:[];}
...
async function start(){
  ...
  app.get('/owner',requireLogin,requireOwner,async(req,res,next)=>{try{const token=csrf(req),site=await store.getGlobalConfig(),codes=(await store.getCodes()).slice(-15).reverse(),guilds=await getBotGuilds();const rows=(await Promise.all(guilds.map(async g=>{const id=String(g.id),cfg=await store.getConfig(id);return `<tr><td>${iconUrl(g)?`<img src="${iconUrl(g)}" alt="" style="width:34px;height:34px;border-radius:10px;vertical-align:middle;margin-left:8px">`:''}${esc(g.name||'Unknown')}<small>${id}</small></td><td>${Number(g.approximate_member_count||0).toLocaleString()}</td><td>${store.isPremium(cfg)?'💎 Premium':'Free'}</td><td><div style="display:flex;gap:8px;flex-wrap:wrap"><form class="mini" method="post" action="/owner/premium"><input type="hidden" name="_csrf" value="${token}"><input type="hidden" name="guildId" value="${id}"><button name="days" value="30">+30d</button><button name="days" value="90">+90d</button><button name="days" value="365">+1y</button><button class="danger" name="days" value="0">Remove Premium</button></form><form class="mini" method="post" action="/owner/guilds/${id}/leave" onsubmit="return confirm('⚠️ سيتم إخراج ZOMBI من هذا السيرفر مباشرة. هل أنت متأكد؟')"><input type="hidden" name="_csrf" value="${token}"><button class="danger" type="submit">🚪 إخراج البوت</button></form></div></td></tr>`;}))).join('');const notice=req.query.left==='1'?'<div class="warn">✅ تم إخراج البوت من السيرفر بنجاح.</div>':req.query.error?`<div class="warn">❌ ${esc(req.query.error)}</div>`:'';res.send(layout('Owner',`<section class="dash-head"><div><h1>👑 Owner Panel</h1><p>${guilds.length} سيرفر موجود فيه البوت الآن</p></div></section>${notice}<section class="panel"><h2>🌐 إعدادات الموقع</h2><form class="form-grid" method="post" action="/owner/site"><input type="hidden" name="_csrf" value="${token}"><label>سعر Premium<input name="premiumPrice" value="${esc(site.premiumPrice)}"></label><label>رابط الشراء<input name="purchaseUrl" value="${esc(site.purchaseUrl)}"></label><label class="wide">إعلان Dashboard<textarea name="announcement">${esc(site.announcement)}</textarea></label><button class="btn primary">حفظ</button></form></section><section class="panel"><h2>Premium Codes</h2><form class="inline-form" method="post" action="/owner/codes"><input type="hidden" name="_csrf" value="${token}"><input type="number" name="days" value="30" min="1" max="3650"><button class="btn">إنشاء كود</button></form><div class="codes">${codes.map(c=>`<div><code>${esc(c.code)}</code><span>${c.days} يوم • ${c.usedAt?'مستخدم':'متاح'}</span></div>`).join('')||'لا يوجد أكواد.'}</div></section><section class="panel table-wrap"><h2>🖥️ السيرفرات الموجود فيها البوت</h2><p class="hint">زر «إخراج البوت» يخلّي ZOMBI يغادر السيرفر مباشرة. إعدادات السيرفر تبقى محفوظة إذا أضفت البوت مرة ثانية.</p><table><thead><tr><th>السيرفر</th><th>الأعضاء</th><th>الخطة</th><th>تحكم</th></tr></thead><tbody>${rows||'<tr><td colspan="4">البوت غير موجود في أي سيرفر.</td></tr>'}</tbody></table></section>`,req.user));}catch(e){next(e);}});
  ...
  app.post('/owner/guilds/:guildId/leave',requireLogin,requireOwner,checkCsrf,async(req,res)=>{try{const guildId=String(req.params.guildId||'').trim();if(!/^\d{15,25}$/.test(guildId))return res.redirect('/owner?error='+encodeURIComponent('Guild ID غير صالح.'));const guild=await getBotGuild(guildId);if(!guild)return res.redirect('/owner?error='+encodeURIComponent('البوت غير موجود في هذا السيرفر.'));await botFetch(`/users/@me/guilds/${guildId}`,{method:'DELETE'});res.redirect('/owner?left=1');}catch(e){console.error('Owner leave guild failed:',e);res.redirect('/owner?error='+encodeURIComponent(e?.message||'تعذر إخراج البوت من السيرفر.'));}});
